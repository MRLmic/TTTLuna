...
var userApi = __webpack_require__(10);
var userUi = __webpack_require__(11);
var store = __webpack_require__(1);

var currentGameArray = [null, null, null, null, null, null, null, null, null];
var accumulator = 2;
var character = 'X';
var indexID = void 0;
...
var fillArray = function fillArray(currentID) {
...
var startOrNah = function startOrNah() {
  if (gameActive === true) {
    $('#wrapper').children().on('click', function (event) {
      ...
      var solSet = [['0', '1', '2'], ['3', '4', '5'], ['6', '7', '8'], ['2', '5', '8'], ['1', '4', '7'], ['0', '3', '6'], ['0', '4', '8'], ['2', '4', '6']];
      solSet.forEach(function (winningCombination) {
        var thisVar = winningCombination.every(function (i) {
...
var changePassword = function changePassword(event) {
  ...
  userApi.changePassword(data).then(userUi.changeSuccess).then($(this).trigger('reset')).catch(userUi.changeFailure);
};
var signUp = function signUp(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.create(data).then(userUi.onSignUpSuccess).then($(this).trigger('reset')).catch(userUi.onSignUpFailure);
};
var signIn = function signIn(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.logIn(data).then(userUi.onSignInSuccess).catch(userUi.onSignInFailure);
};
var signOut = function signOut(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.signOut(data).then(userUi.signOutSuccess).catch(userUi.signOutFailure);
};
var createNewGame = function createNewGame(event) {
  event.preventDefault();
  var data = getFormFields(this);
  userApi.newGame(data).then(userUi.newGameSuccess);
};
var updateGame = function updateGame(indexID, turn, gameOver) {
  var data = {
    'game': {
      'cell': {
        'index': indexID,
...
var getGames = function getGames(event) {
  event.preventDefault();
  userApi.get().then(userUi.onGetSuccess);
};
var resetFormsSignUp = function resetFormsSignUp() {
...
var onSignUpFailure = function onSignUpFailure(data) {
...
var onSignInFailure = function onSignInFailure(data) {
  $('.sign-up-success').text('oops, something went wrong. please try again');
};
var changeSuccess = function changeSuccess(data) {
  $('.turn').text('pw changed');
};
var changeFailure = function changeFailure(data) {
